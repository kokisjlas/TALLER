...
public class Taller {
	...
	private int indicador_vehiculos=0;
	
	private Mecanico[] mecanicos= new Mecanico[100];
	private int indicador_mecanico=0;*/
	...
	/*
	vehiculos[indicador_vehiculos]=nuevo_vehiculo;
	indicador_vehiculos++;*/
	...
	public Mecanico buscarMecanicoLibre(int tipo) {
		/*
		for(int i=0;i<indicador_mecanico;i++) {
			if(mecanicos[i].isLibre()==true) {//Se verifica la especialidad del Mecanico (Coche)
				if(mecanicos[i].getEspecialidad()==tipo) {
					return mecanicos[i];
				}
			}
		}*/
	...
	public Vehiculo vehiculosEnEspera() {
		/*for(int i=0;i<indicador_vehiculos;i++) {
			if(vehiculos[i].getEstado()==TipoEstado.EN_ESPERA) {
				return vehiculos[i];
			}
		}*/
		for(Vehiculo v:vehiculosA) {
	...
	public void listarVehiculos() {
		/*for (int i=0;i<indicador_vehiculos;i++) {
			if(vehiculos[i].getEstado()==TipoEstado.REPARADO) {
				vehiculos[i]=vehiculos[indicador_vehiculos-1];
				indicador_vehiculos--;
			}
	...
	public void altaMecanico(Mecanico nuevomecanico) {
		/*mecanicos[indicador_mecanico]=nuevomecanico;
		indicador_mecanico++;*/
